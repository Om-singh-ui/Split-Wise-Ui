...
export function ExpenseForm({ type = "individual", onSuccess }) {
  ...
  // Add useCallback for participants handling
  const handleParticipantsChange = useCallback(
    (newParticipants) => {
      // Ensure current user is always included
      const currentUserIncluded = newParticipants.some(
  ...
  // Add useEffect to update paidBy when participants change
  useEffect(() => {
    const paidBy = watch("paidByUserId");
    if (!paidBy || !participants.some((p) => p.id === paidBy)) {
      const defaultPayer = currentUser?._id || participants[0]?.id;
      if (defaultPayer) {
        setValue("paidByUserId", defaultPayer);
      }
    }
  }, [participants, watch, setValue, currentUser]);
  ...
  const handleGroupChange = useCallback(
    (group) => {
      ...
      // Always include all group members
